 ...
 */
int builtin_cmd(char **argv) {
/*判断该命令是否为内置指令，若是则立即执行反之则返回。*/
  if (!strcmp(argv[0], "quit")) { /* 判断是否为quit指令 exit(0); */ exit(0);}
  if (!strcmp(argv[0], "&")) { /* 判断若为& */    return 1;}
  if (!strcmp(argv[0], "fg") || !strcmp(argv[0], "bg")) { /* 判断为fg或bg指令 */
    do_bgfg(argv);/*执行对应信息*/
    return 1;
  }
  if (!strcmp(argv[0], "jobs")) { /* 判断为jobs指令 */
...
/* 执行内置命令bg fg */
  ...
  if (isdigit(argv[1][0])) {/*以数字开头*/
    pid_t pid = atoi(argv[1]);
    if (!(jobp = getjobpid(jobs, pid))) {
      printf("(%d): No such process\n", pid);
      return;
    }
  } else if (argv[1][0] == '%') {/*以%开头*/
    int jid = atoi(&argv[1][1]);/*取地址*/
  ...
  /* bg command */
  if (!strcmp(argv[0], "bg")) {
    if (kill(-(jobp->pid), SIGCONT) < 0) unix_error("kill (bg) error");/*向job所在的进程组发送SIGCONT信号*/
    jobp->state = BG;/*修改状态*/
  ...
  /* fg command */
    ...
    if (kill(-(jobp->pid), SIGCONT) < 0) unix_error("kill (fg) error");/*向job所在的进程组发送SIGCONT信号*/
    jobp->state = FG;/*修改状态*/
    waitfg(jobp->pid);/*等待，直到前台进程修改*/
  } else {
 ...
 */
void waitfg(pid_t pid) {
  while (pid == fgpid(jobs)) { sleep(0); /* 调用sleep（0）可以释放cpu时间，让线程马上重新回到就绪队列而非等待队列 */ }
  return;
}
...
/*父进程中接收到 SIGCHLD 信号的处理函数。*/
  ...
  if (sigfillset(&mask) < 0) unix_error("sigfilldset error");/*填入set集合*/

  while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
/*在while循环中使用waitpid()函数，以尽可能多的回收僵尸进程。*/
    jobp = getjobpid(jobs, pid); /* 根据pid获取子进程对应的job_t */
    if (sigprocmask(SIG_BLOCK, &mask, &Old_mask) <
        0) /* 阻塞信号 */
      unix_error("sigprocmask error");

    if (WIFSTOPPED(status)) { /* 该进程已停止 */
      jobp->state = ST;
      printf("Job [%d] (%d) stopped by signal %d\n", jobp->jid, jobp->pid,
             WSTOPSIG(status));/*打印对应信息*/
    } else {
      if (WIFSIGNALED(status)) /* 若是子进程终止引起的返回 */
        printf("Job [%d] (%d) terminated by signal %d\n", jobp->jid, jobp->pid,
               WTERMSIG(status));
      deletejob(jobs, pid); /*将其回收*/
    }

...
SEQUENCE_LENGTH = 256
# 测试开关
DEBUG_DELAY_ON = False
DEBUG_RANDOM_THROW_ON = False
...
class StopwaitReceiver:
    ...
    def __receive(self, addr):
        ...
        if not rlist:
            return ''
        bytes, _ = self.__socket.recvfrom(BUFFER_SIZE)
        message: str = bytes.decode()
    ...
    def begin_receive(self, addr):
        ...
        fo = open("D:/大三/WebLab/Lab2_GBN&SR/content.txt","w")
        fo.write(final.format(self.__name, message))
        fo.close()

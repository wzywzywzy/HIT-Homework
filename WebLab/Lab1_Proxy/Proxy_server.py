...
class ProxyServer:
    def __init__(self,
        ...
        with open(config_file) as file:
            ...
            #  read as dictionary
            self.forbidden_ip_list = config["forbidden_user"]
            self.forbidden_web_list = config["forbidden_web"]
    ...
    def _read_from_cache(self, url):
        """读取缓存文件
        if the chche doesn't exist return false with cache_file
        if the content hadn't changed until now return the true with the cache
        else return flase with cache_file
        """
    ...
    def forbidden_ip(self, ip):
        """判断 IP 地址是否为需过滤的 IP 地址
        """
        return self.forbidden_ip_list and any(
            [re.match(regex, ip) for regex in self.forbidden_ip_list])

    def forbidden_hostname(self, hostname):
        """判断主机名是否为需要过滤的主机命
        """
        return self.forbidden_web_list and any(
            [re.match(regex, hostname) for regex in self.forbidden_web_list])
    ...
    def proxy_connect(self, proxy_socket: socket.socket, address):
        ...
        for header in headers:
            if re.match(header[:4].lower(),'host'):
                hostname = header[6:].split(':')[0]
                if len(header[6:].split(':')) == 2:
                    port = int(header[6:].split(':')[1])
                break
        if not hostname:
        ...
        if self.forbidden_hostname(hostname):
        ...
        if self.forbidden_ip(address[0]):
